...
const postTemplate = ({ id, body, by, username, href, time, children }) => `
  <div class="post" id="post-${id}">
    <div class="date pullout"></div>
    <p class="body">${body}</p>
    <div class="info">
      <span class="by">${by}</span>
      <a class="username" href="${href}">${username}</a>
      alle
...
function callback(json) {
  json.rows.forEach(function (i, row) {
    var post = row.value;
    lastLoaded = row.key;
    ...
    var el = postTemplate({
      id: post._id,
      body: post.body,
      by: by[Math.floor(Math.random() * by.length)],
      username: post.user,
      href: "/user/#" + post.user,
    ...
    if (post.comments.length) {
      el.find(".commentLink:first")
        .text(post.comments.length + " commenti")
        .attr("href", "#post-" + post._id);
    }
    $.each(post.comments, function (i, comment) {
      el.find(".comment.template")
        .use()
        .find(".body:first")
        .html(comment.body)
        .end()
        .find(".by:first")
        .html(by[Math.floor(Math.random() * by.length)])
        .end()
        .find(".username:first")
        .text(comment.user)
        .attr("href", "/user/#" + comment.user)
        .end()
        .find(".time:first")
        .text(getTime(comment.date))
        .end()
        .find(".date:first")
